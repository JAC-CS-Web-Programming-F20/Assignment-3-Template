...
use AssignmentThree\Controllers\PostController;
use AssignmentThree\Exceptions\PostException;
use AssignmentThree\Router\Request;
use AssignmentThree\Router\Response;
use AssignmentThreeTests\AssignmentThreeTest;

final class PostControllerTest extends AssignmentThreeTest
{
	...
	public function testExceptionWasThrownShowingPost(string $exception, string $message, string $requestMethod, array $parameters): void
	{
		...
		(new PostController($request))->doAction();
	}
	...
	public function findPostProvider()
	{
		yield 'invalid ID' => [
			PostException::class,
			'Cannot show Post: Post does not exist with ID 1.',
			'GET',
			[
				'body' => [],
				'header' => [1]
