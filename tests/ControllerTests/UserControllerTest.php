...
use AssignmentThree\Controllers\UserController;
use AssignmentThree\Exceptions\UserException;
use AssignmentThree\Router\Request;
use AssignmentThree\Router\Response;
use AssignmentThreeTests\AssignmentThreeTest;

final class UserControllerTest extends AssignmentThreeTest
{
	...
	public function testExceptionWasThrownShowingUser(string $exception, string $message, string $requestMethod, array $parameters): void
	{
		...
		(new UserController($request))->doAction();
	}
	...
	public function findUserProvider()
	{
		yield 'invalid ID' => [
			UserException::class,
			'Cannot show User: User does not exist with ID 1.',
			'GET',
			[
				'body' => [],
				'header' => [1]
